...
<script type="text/javascript">
		function openUserSelect2() {
		   ...
		   $.dialog({content: 'url:userController.do?userselectauto', zIndex: 2105, title: '用户列表', lock: true, width: '827px', height: '650px', opacity: 0.4, button: [
		...
		function openUserSelect3() {
			   $.dialog.setting.zIndex = 9999; 
			   $.dialog({content: 'url:userController.do?userselectauto', zIndex: 2105, title: '用户列表', lock: true, width: '600px', height: '400px', opacity: 0.4, button: [
			      {name: '<t:mutiLang langKey="common.confirm"/>', callback: callbackUserSelect2, focus: true},
			      {name: '<t:mutiLang langKey="common.cancel"/>', callback: function (){}}
			   ]}).zindex();
			}
		function callbackUserSelect2() {
			var iframe = this.iframe.contentWindow;		
			var rowsData = iframe.$('#userList1').datagrid('getSelections');
			var realname=rowsData[0].realName;
			var id=rowsData[0].id;
			$('#createname').val(realname);
			$('#createid').val(id);		
			}
		$(function(){
...
<t:formvalid formid="formobj" layout="div" dialog="true" action="processAgentController.do?saveProcessAgent">
	<table style="width: 600px;" cellpadding="0" cellspacing="1"  >
		...
		<tr >
			...
			<td class="value"><input  class="inputxt"    value=<%=ResourceUtil.getSessionUserName().getRealName() %> id="createname" onclick="openUserSelect3()"> 
