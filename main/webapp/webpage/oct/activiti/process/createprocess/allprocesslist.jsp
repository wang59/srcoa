...
function collection(id,type){
	 $.ajax({
			url : "createAppController.do?collection",
			data : {
				"id" : id,
				"type":type,			
			},
...
<body>
		...
		<!-- 循环取出类别 -->
			<c:forEach var="pro" items="${pro_map}">
				<h2 class="clear">${pro.key }</h2>
				<!-- 循环取出类别下的流程 -->
				<c:forEach var="impl" items="${pro.value }" varStatus="b">
					<c:if test="${ (b.count-1)%4!=0}">
						<div class="li">
							<a href="${impl.url}&proid=${impl.id}" target="_blank">${impl.name }</a>
                             <c:if test="${!impl.collection }"><a onclick="collection('${impl.id }','${impl.type.id}')"><i class="icon-heart-empty" title="加入收藏"></i></a></c:if>
						</div>
					</c:if>
					<c:if test="${ (b.count-1)%4==0}">
...
<script>
$(function(){
	$('.icon-heart-empty').mouseenter(function(){
