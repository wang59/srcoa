...
<t:datagrid name="userList" title="common.operation" actionUrl="userController.do?datagrid" 
	...
	<t:dgToolBar title="common.password.reset" icon="icon-edit" url="userController.do?changepasswordforuser" funname="update2"></t:dgToolBar>
...
function update2(title,url, id,width,height,isRestful) {
	...
	createwindow2(title,url,width,height);
}
...
function saveObj2() {
	var psd=iframe.$("#repassword").val();
	var id=iframe.$("#id").val();
	$.ajax({
		url : "oldUserController.do?savenewpwdad",
		data : {
			"psd" : psd,
			"id":id
		},
