 var addTabsIFrame = function (options) {
    ...
    var last = $("#tabs>ul>li:last", parent.document);
        ...
        items: [
            {
                ...
                var div = $("#tabs", parent.document).find("div[id='"+tab+"']");
            ...
            },
            {
                ...
                var nowText = $("#tabs", parent.document).find("li[class='active']").children("a").text().trim();
                if(closeText==nowText){
                    ...
                    if(prevCount==0){
                        var tab = last.children("a").attr("aria-controls").toString();
                        last.remove();
                        $("#tabs", parent.document).find("div[id='"+tab+"']").remove();
                    }else{
                        //显示前一个tab
                        var tab = last.children("a").attr("aria-controls").toString();
                        ...
                        $("#tabs", parent.document).find("div[id='"+id+"']").addClass("active");
                    }
                }else{
                    //关闭的不是当前页，关闭就好了╮(╯_╰)╭
                    var tab = last.children("a").attr("aria-controls").toString();
            ...
            },
            {
                ...
                $("#tabs>ul>li", parent.document).remove();
                $("#tabs>div>div", parent.document).remove();
                //tabs.tabs("refresh");
            }
            },
            {
                ...
                if(closeText==nowText){
                    //此是当前页面
                    var tab = last.children("a").attr("aria-controls").toString();
                    $("#tabs>ul>li", parent.document).not(last).remove();
                    $("#tabs>div>div", parent.document).not($("#tabs").find("div[id='"+tab+"']")).remove();
                }else{
                    var tab = last.children("a").attr("aria-controls").toString();
                    $("#tabs>ul>li", parent.document).not(last).remove();
                    $("#tabs>div>div", parent.document).not($("#tabs", parent.document).find("div[id='"+tab+"']")).remove();
            ...
            null,
            {
                ...
                if(closeText==nowText){
                    ...
                    if(nextAll.length!=0){
                        ...
                        $("#tabs>div", parent.document).find("div[id='"+tab+"']").nextAll().remove();
                        //tabs.tabs("refresh");
                    }else{
                        layer.msg('<b>右侧没有啦</b>');
                    }
                    ...
                    //不是当前页，当前页的active去掉
                    var now = $("#tabs", parent.document).find("li[class='active']");
                    var nowid = now.children("a").attr("aria-controls").toString();
                    now.removeClass("active");
                    $("#tabs", parent.document).find("div[id='"+nowid+"']").removeClass("active");
                    var nextAll = last.nextAll();
                    if(nextAll.length!=0){
                        nextAll.remove();
                        var tab = last.children("a").attr("aria-controls").toString();
            ...
            },
            {
                ...
                if(closeText==nowText){
                    ...
                    if(prevAll.length!=0){
                        ...
                        var tab = last.children("a").attr("aria-controls").toString();
                        //$("#tabs>ul>li").not(shouye).remove();
                        $("#tabs>div", parent.document).find("div[id='"+tab+"']").prevAll().remove();
                        //tabs.tabs("refresh");
                    }else{
                        layer.msg('<b>左侧没有啦</b>');
                    }
...
var closeTab = function (id) {
    ...
    if ($("li.active", parent.document).attr('id') == "tab_" + id) {
...
$(function () {
    mainHeight = $(document.body).height();
    $('.main-left,.main-right', parent.document).height(mainHeight);
    $("[addtabs]", parent.document).click(function () {
        addTabs({ id: $(this).attr("id"), title: $(this).attr('title'), close: true });
    });

    $(".nav-tabs", parent.document).on("click", "[tabclose]", function (e) {
